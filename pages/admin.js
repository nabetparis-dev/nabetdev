...
function emptyProduct(categories=[]){
  return {id:"",name:"",slug:"",categoryId:categories[0]?.id||"",price:0,oldPrice:0,stock:20,badge:"מבצע",color:"",image:"",images:[],shortDescription:"",description:"",seoTitle:"",seoDescription:"",rating:5,reviewCount:36,reviews:[],showStockUrgency:true,urgentText:""};
}
...
export default function Admin(){
  ...
  const [full,setFull]=useState({products:[],categories:[],orders:[],customers:[],coupons:[],media:[],content:{},pagesCms:{},apiSettings:{},socialSettings:{},designSettings:{},translations:{}});
  ...
  async function saveProduct(){
    const p={...currentProduct}; if(!p.id)p.id=makeSlug(p.name); if(!p.slug)p.slug=makeSlug(p.name||p.id); if(!p.image&&p.images?.length)p.image=p.images[0]; if(!p.seoTitle)p.seoTitle=`${p.name} | NABET PARIS ישראל`; if(!p.seoDescription)p.seoDescription=p.shortDescription||p.description||"";
  ...
  const revenue=(full.orders||[]).reduce((s,o)=>s+Number(o.total||0),0); const pending=(full.orders||[]).filter(o=>o.status==="pending").length; const lowStock=(full.products||[]).filter(p=>Number(p.stock||0)<=3).length;
  ...
  return <main className="admin wcAdmin">
    ...
    {tab==="products"&&<section className="adminBox"><div className="boxHead"><h2>Produits</h2><button onClick={()=>setCurrentProduct(emptyProduct(full.categories))}>+ Ajouter un produit</button></div><div className="adminList productList">{(full.products||[]).map(p=><div key={p.id} className="adminItem"><img src={(p.images&&p.images[0])||p.image||"/products/placeholder-wallet.svg"} onError={e=>{e.currentTarget.src="/products/placeholder-wallet.svg"}}/><b>{p.name}</b><span>{shekel(p.price)} — Stock {p.stock}</span><button onClick={()=>setCurrentProduct({...p,images:p.images||(p.image?[p.image]:[])})}>Modifier</button><button onClick={()=>duplicateProduct(p)}>Dupliquer</button><button onClick={()=>deleteProduct(p.id)}>Supprimer</button></div>)}</div></section>}
...
return <div className="modalShade" onClick={close}><div className="adminModal productModal" onClick={e=>e.stopPropagation()}><button className="modalClose" onClick={close}>×</button><h2>{p.id?"Modifier produit":"Ajouter produit"}</h2><div className="twoCols"><label>Nom produit<input value={p.name||""} onChange={e=>set("name",e.target.value)}/></label><label>ID / URL<input value={p.id||""} onChange={e=>set("id",e.target.value)} placeholder="automatique si vide"/></label><label>Prix<input type="number" value={p.price||0} onChange={e=>set("price",Number(e.target.value))}/></label><label>Ancien prix<input type="number" value={p.oldPrice||0} onChange={e=>set("oldPrice",Number(e.target.value))}/></label><label>Stock restant<input type="number" value={p.stock||0} onChange={e=>set("stock",Number(e.target.value))}/></label><label>Catégorie<select value={p.categoryId||""} onChange={e=>set("categoryId",e.target.value)}>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label><label>Couleur<input value={p.color||""} onChange={e=>set("color",e.target.value)} placeholder="שחור / כחול / אדום..."/></label><label><input type="checkbox" checked={p.showStockUrgency!==false} onChange={e=>set("showStockUrgency",e.target.checked)}/> Afficher urgence stock</label></div><label>Texte urgence personnalisé<textarea value={p.urgentText||""} onChange={e=>set("urgentText",e.target.value)} placeholder="האחרון במלאי — אל תפספסו"/></label><label>Sous-titre<textarea value={p.shortDescription||""} onChange={e=>set("shortDescription",e.target.value)}/></label><label>Description longue<textarea value={p.description||""} onChange={e=>set("description",e.target.value)}/></label><div className="imageAdminBox"><h3>Images produit</h3><label className="uploadDrop">{uploading?"Upload...":"+ Ajouter images depuis mon Mac"}<input type="file" multiple accept="image/*" onChange={e=>addImages(e.target.files)}/></label><div className="imagePreviewGrid">{(p.images||[]).map((img,i)=><div key={i} className="imagePreviewItem"><button className="deleteImg" onClick={()=>removeImage(i)}>×</button><img src={img} onError={e=>{e.currentTarget.src="/products/placeholder-wallet.svg"}}/><button onClick={()=>mainImage(i)}>{p.image===img||i===0?"ראשית":"הפוך לראשית"}</button></div>)}</div><details><summary>Choisir depuis Médias</summary><div className="miniMediaGrid">{media.slice(0,60).map(m=><button key={m.id} onClick={()=>addMedia(m.url)}><img src={m.url}/></button>)}</div></details><label>URLs galerie<textarea className="galleryTextarea" value={(p.images||[]).join("\n")} onChange={e=>{const images=e.target.value.split(/\n|,/).map(x=>x.trim()).filter(Boolean); setP({...p,images,image:images[0]||p.image})}}/></label></div><div className="twoCols"><label>SEO title<input value={p.seoTitle||""} onChange={e=>set("seoTitle",e.target.value)}/></label><label>SEO description<textarea value={p.seoDescription||""} onChange={e=>set("seoDescription",e.target.value)}/></label><label>Étoiles<input type="number" step="0.1" value={p.rating||5} onChange={e=>set("rating",Number(e.target.value))}/></label><label>Nombre avis<input type="number" value={p.reviewCount||0} onChange={e=>set("reviewCount",Number(e.target.value))}/></label></div><div className="modalActions"><button onClick={save}>Sauvegarder produit</button><button onClick={close}>Annuler</button></div></div></div> }
